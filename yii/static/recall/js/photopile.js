...
function PhotoPile(options) {
    ...
    var config, defaultOptions = {
        ...
        // Images
        loading: 'static/recall/images/loading.gif' // path to img displayed while gallery/thumbnails loads
    };
    ...
    var thumb = {
        ...
        setRandomZ: function (thumb) {
            thumb.css({
                'z-index': Math.floor((Math.random() * config.numLayers) + 1)
            });
        },
        ...
        clearActiveClass: function () {
            $('li.' + this.active).fadeTo(config.fadeDuration, 1).removeClass(this.active);
        }
    ...
    var photo = {
        ...
        showInfo: function () {
            if (config.showInfo) {
                var title = thumb.getActive().children('a').children('img').attr('title');
                if (title == undefined || title == '' || title == null) title = '';
                var alt = thumb.getActive().children('a').children('img').attr('alt');
                if (alt == undefined || alt == '' || alt == null) alt = '';
                this.info.children().html(alt + '<span style="float:right">' + title + '</span>');
                this.info.css({
